...
fake_DEF = subset(fake_DEF, DEF > 7 & THEfinalRULE == 2)
dim(fake_DEF)[1]

```
High DEF and high VAL
```{r}
fake_DEF_VAL = fake[c("DEF", "THEfinalRULE", "VAL")]
fake_DEF_VAL = subset(fake_DEF_VAL, DEF > 7 & VAL > 4 & THEfinalRULE == 2)
dim(fake_DEF_VAL)[1]
```
